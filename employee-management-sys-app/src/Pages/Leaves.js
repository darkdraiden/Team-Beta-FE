  ...
  const fetchLeaveCount = async () => {
    try {
      if (member && member.memberId) {
        ...
        const response = await axios.get(`http://localhost:8080/member/leaves/${member.memberId}`,{headers:{'Authorization':bearerToken}});
  ...
  const handleApplyLeave  = async () => {
    try {
      if (member && member.memberId) {
        ...
        console.log("leave token"+token);
        // const response = await axios.get(`http://localhost:8080/member/leaves/${member.memberId}`,{headers:{'Authorization':bearerToken}});
        const response = await axios.put(`http://localhost:8080/member/leaves/${member.memberId}`,{},{headers:{'Authorization':bearerToken}});
        // setLeaveCount(response.data);
      //  fetchLeaveCount();
